...
[IncomingMessage(15)]
public class UpdateFriendslistMessage : IncomingMessage
{
    public UpdateFriendslistMessage(MessageHeader header, string raw) : base(header, raw)
    {
    }
    ...
    {
        if(ctx.User is null)
            return new UpdateFriendslistResponseMessage(ctx, []);
        ...
        return new UpdateFriendslistResponseMessage(
            ctx,
            responseData
        );
    }
