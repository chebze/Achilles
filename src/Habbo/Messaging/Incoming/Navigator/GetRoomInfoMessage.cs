...
[IncomingMessage(21)]
public class GetRoomInfoMessage : IncomingMessage
{
    public int RoomId { get; set; }

#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    public GetRoomInfoMessage(MessageHeader header, string raw) : base(header, raw)
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider adding the 'required' modifier or declaring as nullable.
    ...
    public override async Task<OutgoingMessage?> RespondAsync(IncomingMessageContext ctx)
    {
        ...
        if(room is null)
            return new AlertMessage("Room not found");

        return new SendRoomInfoMessage(ctx, room);
    }    
}
