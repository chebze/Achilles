...
public abstract class BaseRepository<TModelType, TPrimaryKeyType> : 
    IBaseRepository<TModelType, TPrimaryKeyType> where TModelType : BaseModel<TPrimaryKeyType>
{
    ...
    public BaseRepository(HabboDbContext context)
    {
    ...
    private async Task<List<TModelType>> GetFromDatabase(Expression<Func<TModelType, bool>> predicate)
    {
        ...
        List<TModelType> resultsFromDatabase = await this._context.Set<TModelType>().Where(predicate).ToListAsync();
